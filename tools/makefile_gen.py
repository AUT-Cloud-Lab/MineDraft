FILE_PATH = "Makefile.iter"

KIND = "normal_multiple"

SCRIPT_NAMES = [
    "average_latency_boxplot",
    "average_latency_linechart",
    "edge_utilization_linechart",
    "placement_ratio_linechart",
    "pod_count_linechart",
    "migration_count_boxplot",
    "average_latency_table",
    # "average_f_linechart",
    # "average_f_table",
]

REPORT_FILE_COUNT = 10

REPORT_DIR_NAMES = [
    # "normal_scenario_1.1_0.4",
    # "normal_scenario_1.2_0.4",
    "normal_scenario_1.3_0.4",
    "normal_scenario_1.4_0.4",
    # "normal_scenario_1.5_0.4",
    # "normal_scenario_1.6_0.4",
]

meta_scripts = [
    "migration_count_boxplot",
    "average_latency_table",
    "average_f_table"
]

def generate_result_str(kind, script_name, report_dir_names, report_file_count):
    result_str = f"{kind}_{script_name}:\n"

    for report_dir_name in report_dir_names:
        parent_dir_name, child_dir_name = script_name.rsplit("_", 1)

        result_dir_name = f"./results/{parent_dir_name}/{child_dir_name}"

        if script_name not in meta_scripts:
            result_dir_name = f"{result_dir_name}/{report_dir_name}"

        history_paths = r""""""
        for it in range(1, report_file_count + 1):
            report_filename = f"{report_dir_name}_{it}.json"
            # base_history_paths = rf"        reports/kube-schedule/$(date_kube_schedule)/{report_dir_name}/{report_filename} \
            #     reports/random-scheduler/$(date_random)/{report_dir_name}/{report_filename} \
            #     reports/cloud-first-scheduler/$(date_cloud_first)/{report_dir_name}/{report_filename} \
            #     reports/smallest-edge-first-scheduler/$(date_smallest_edge_first)/{report_dir_name}/{report_filename} \
            #     reports/biggest-edge-first-scheduler/$(date_biggest_edge_first)/{report_dir_name}/{report_filename} \
            #     reports/ecmus-all-half/$(date_ecmus_all_half)/{report_dir_name}/{report_filename} \
            #     reports/ecmus-only-d/$(date_ecmus_only_d)/{report_dir_name}/{report_filename} \
            #     reports/ecmus-c-gt-a/$(date_ecmus_c_gt_a)/{report_dir_name}/{report_filename} " + "\\"

            base_history_paths = rf"        reports/ecmus-qos-aware/$(date_ecmus_qos_aware)/{report_dir_name}/{report_filename} \
                reports/ecmus-no-cloud-offload/$(date_ecmus_no_cloud_offload)/{report_dir_name}/{report_filename} \
                reports/ecmus-no-edge-migration/$(date_ecmus_no_edge_migration)/{report_dir_name}/{report_filename} \
                reports/ecmus-no-migration/$(date_ecmus_no_migration)/{report_dir_name}/{report_filename} \
                reports/ecmus-mid-migration/$(date_ecmus_mid_migration)/{report_dir_name}/{report_filename} " + "\\"

            if it != report_file_count:
                base_history_paths += "\n"

            history_paths += base_history_paths

        script_label_name = script_name

        # METASCRIPTS
        if script_name == "migration_count_boxplot":
            script_label_name = "migration_count_metadata"

        if script_name == "average_latency_table":
            script_label_name = "average_latency_metadata"

        if script_name == "average_f_table":
            script_label_name = "average_f_metadata"

        command_str = "\t python3 main.py \\\n" + rf"""    --script_name {script_label_name} \
            --config_path config.json \
            --history_paths \
        {history_paths}
            --scenario-name {report_dir_name} \
            --save-path {result_dir_name}
        """

        command_str += "\n"
        result_str += command_str

    if script_name in meta_scripts:
        command_str = "\t python3 main.py \\\n" + rf"""    --script_name {script_name} \
            --config_path config.json \
            --save-path {result_dir_name}
        """

        command_str += "\n"
        result_str += command_str

    return result_str


if __name__ == "__main__":
    with open(FILE_PATH, "a") as file:
        for script_name in SCRIPT_NAMES:
            result_str = generate_result_str(KIND, script_name, REPORT_DIR_NAMES, REPORT_FILE_COUNT) + "\n"
            file.write(result_str)
