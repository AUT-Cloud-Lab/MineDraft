...
def parse_config(json_data: dict) -> Config:
    ...
    for deployment_data in json_data["deployments"]:
        deployment = Deployment(deployment_data["name"], list(map(float, deployment_data["resources"])))
        deployments[deployment.name] = deployment
    
    for node_data in json_data["nodes"]:
        node = Node(node_data["name"], node_data["is_on_edge"])
...
def parse_pod_placement(config: Config, json_data: dict) -> PodPlacement:
    ...
    for node_name, pod_names in json_data["node_pods"].items():
        ...
        for pod_name in pod_names:
            ...
            if deployment_name not in config.deployments:
                continue    
            deployments.append(config.deployments[get_deployment_name_from_pod_name(pod_name)])
...
def parse_hpa_state(config: Config, json_data: dict) -> HpaState:
    ...
    for deployment_name_in_hpa, metric in json_data["deployment_metrics"].items():
